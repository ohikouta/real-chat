...
export function getStorageErrorMessage(error, fallback = 'ファイル操作に失敗しました。時間を置いて再度お試しください。') {
  switch (error?.code) {
    case 'storage/unauthorized':
      return 'ファイルを操作する権限がありません。';
    case 'storage/canceled':
      return 'ファイル操作がキャンセルされました。';
    case 'storage/retry-limit-exceeded':
      return 'ファイル操作がタイムアウトしました。時間を置いて再度お試しください。';
